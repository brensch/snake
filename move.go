...
func Move2(ctx context.Context, state *rules.BoardState, ruleset rules.Ruleset, you rules.Snake, turn int32, gameID string) (rules.Direction, string) {
	...
	// startingNode.Search(ctx, 14, ruleset)
	// bestNextState := startingNode.DeepeningSearch(ctx, ruleset)
	bestChild, score := minimax.Search2(state, 15, math.Inf(-1), math.Inf(1), true, ruleset)
	fmt.Println(score)
